...
export const OkResponse = <T extends FastifySchema>(
  reply: FastifyReplyTypeBox<T>,
  data: T["response"] extends { 200: infer U }
    ? U extends TSchema
      ? Static<U>["data"]
      : never
    : never,
...
export const CreatedResponse = <T extends FastifySchema>(
  reply: FastifyReplyTypeBox<T>,
  data: T["response"] extends { 201: infer U }
    ? U extends TSchema
      ? Static<U>["data"]
      : never
    : never,
...
export const NoContentResponse = <T extends FastifySchema>(
  reply: FastifyReplyTypeBox<T>,
) => {
